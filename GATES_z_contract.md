...
- [x] Z19: POST-RUN CORRECTION - the four folds are NESTED expanding windows, so the pre-registered "179 training days" is a sum over 51 distinct days and the z=6 coverage anchor was inflated (99.4% stated, 98.0% actual)
  ...
  EVIDENCE: exit=0 expect-matched sha256:4182de06f360 11.4s

- [x] Z20: POST-RUN - the Gamma effect exceeds the solver's own run-to-run spread; z=4 re-run for Gamma 0 and 2 on all four folds at two further seeds, giving three samples per cell
  CHECK: C:\ermal\Virtual_Environment_LocalSolver_3\Scripts\python.exe tools\checks_z.py z20
  EXPECT: Z20 PASS gamma-effect-exceeds-seed-noise
  EVIDENCE: exit=0 expect-matched sha256:52ffd7fb8d30 0.9s
