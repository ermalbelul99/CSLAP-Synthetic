...
CORRECTION (post-run, 2026-08-31). The table below was pre-registered over
"179 training days". That figure is a SUM OVER NESTED FOLDS: the folds are
expanding windows, 38 < 43 < 47 < 51 days all beginning 2021-09-01, so the
earliest 38 days are counted four times. There are only 51 DISTINCT training
days, 18 distinct test days, and 56 days of record in total (~11 weeks, one
site). The corrected right-hand column below is what the anchors actually are.
The pre-registered numbers are kept verbatim so the change is auditable.

Measured anchors (incumbent layout, 24 stations):

  z    station-days within  whole days clean   whole days clean
       allowance (179)      (179 nested)       (51 DISTINCT)      <- honest
  2              96.74%            49.7%             51.0%
  2.7            98.70%            74.3%             70.6%
  3              99.07%            80.4%             80.4%
  4              99.77%            94.4%             94.1%
  6              99.98%            99.4%             98.0%        <- was inflated
  8             100.00%           100.0%            100.0%

The z=6 anchor matters most: z=6 was justified as "matches demonstrated status
quo (99.4%)". On distinct days it is 98.0%, i.e. the incumbent leaves ONE of 51
days unclean, and a Clopper-Pearson interval on 1/51 admits a true unclean rate
up to ~10%. The z window is a point estimate from 51 days with no interval, and
its upper bound z <= 6.07 is a MAXIMUM set by a single day. Treat the window as
indicative, not as a calibrated tolerance.

OUT-OF-SAMPLE SIZE, which the pre-registration never stated: 18 test days
(5+4+4+5) over four consecutive weeks. With four independent weekly units the
smallest attainable two-sided sign-test p is 0.125, so NO comparison in this
study can reach p < 0.05 on weekly units by construction.
...
- [x] Z1: the declared z window is bounded by measured anchors, not chosen - the incumbent needs z=6.07 in-sample and the median station needs ~2.7
  ...
  EVIDENCE: exit=0 expect-matched sha256:6a5a33b7e95c 42.9s
...
- [x] Z2: station shares form a complete carve-up of each day - they sum to exactly 1.000 on every training day of every fold
  ...
  EVIDENCE: exit=0 expect-matched sha256:2faf76c0e76e 39.2s
...
- [x] Z3: the allowance identity holds exactly - total permission is 1 + z*sum(sigma), so it always exceeds 100% of the day and aggregate infeasibility is impossible by construction
  ...
  EVIDENCE: exit=0 expect-matched sha256:89a86a5330b0 46.2s
...
- [x] Z4: no leakage - mu and sigma computed from training days reproduce exactly when test data is withheld entirely
  ...
  EVIDENCE: exit=0 expect-matched sha256:ae6dbcb44086 42.3s
...
- [x] Z5: the contract discriminates in BOTH directions - the incumbent passes at z=6.07 and FAILS at z=2, so the oracle is not vacuous
  ...
  EVIDENCE: exit=0 expect-matched sha256:4e0ac593f8fc 40.1s
...
- [x] Z6: NEGATIVE CONTROL - a deliberately concentrated layout (highest-volume products piled onto the largest station) is rejected at every declared z, proving the constraint still prevents the pathology it exists for
  ...
  EVIDENCE: exit=0 expect-matched sha256:b78fcff425ff 62.5s
...
- [x] Z7: the budget arithmetic leaves room for the declared Gamma range - protection at Gamma=4 fits inside the slack at every declared z, measured on the QUIETEST training day (the binding one: protection is a constant number of lines while the allowance scales with the day's volume, so checking the busiest day overstates affordability by the 2.6x max/min volume ratio)
  ...
  EVIDENCE: exit=0 expect-matched sha256:eab744d8d094 40.3s
...
- [x] Z9: PHASE 3 - beta tightens the share allowance and no longer uses the homogeneous-speed formula that produced a flat ceiling of 13 against revealed ceilings spanning 0.015 to 87. Part (b) of this gate was REPLACED after the Phase 7 grid, and the replacement is flagged as the one assertion changed after seeing data.
  ...
  EVIDENCE: exit=0 expect-matched sha256:882c1633efe0 84.2s
  NOTE: the original part (b) required every beta arm's objective to exceed
    ...
    suboptimality on this grid is therefore AT LEAST 1.79% -- an inversion is
    a LOWER bound on gamma0's distance from the optimum, never an upper bound
    on solver error. The only available upper bound is Hexaly's own dual, and
    it is vacuous here: across the 70 feasible rows bound/obj runs 0.000 to
    0.233 (median 0.073), so no arm is certified within 77% of optimal.
    CONSEQUENCE:
    ...
    (7-15%) lies above it. THE SAME RULE APPLIES SYMMETRICALLY and was at
    first applied only to beta: the visit gap between Gamma=2 and the
    INCUMBENT at z=3 is 1.34% (per fold 0.95-1.47%), also below the 1.79%
    floor, and must likewise not be reported as a distinguishable saving.
    At z=4 (3.25%) and z=6 (5.81%) it clears the floor.
- [x] Z10: PHASE 6 - the pilot cell completes and its layout honours the contract on every training day
  CHECK: C:\ermal\Virtual_Environment_LocalSolver_3\Scripts\python.exe tools\checks_z.py z10
  EXPECT: Z10 PASS pilot-honours-contract
  EVIDENCE: exit=0 expect-matched sha256:b0d7eaf03e5a 1.8s
...
- [x] Z11: PHASE 7 - all twelve grid cells complete and write six arms each
  CHECK: C:\ermal\Virtual_Environment_LocalSolver_3\Scripts\python.exe tools\checks_z.py z11
  EXPECT: Z11 PASS grid-complete
  EVIDENCE: exit=0 expect-matched sha256:7cae8c9ba8ff 1.0s
...
- [x] Z12: every solved layout satisfies its own contract on the training days it was optimised against, AND the set of scored layouts matches exactly the set of arms recorded feasible (an arm that returns no layout writes no file, so without this the gate would pass vacuously on whatever did solve - the same defect class as the all-zero demand matrix that made an earlier gate vacuous)
  CHECK: C:\ermal\Virtual_Environment_LocalSolver_3\Scripts\python.exe tools\checks_z.py z12
  EXPECT: Z12 PASS layouts-honour-contract
  EVIDENCE: exit=0 expect-matched sha256:e7bb4b8b982a 1.8s
...
- [x] Z14: Gamma_bind and Gamma_cover are reported per station, so the paper states WHERE protection is useful rather than only whether an arm won
  CHECK: C:\ermal\Virtual_Environment_LocalSolver_3\Scripts\python.exe tools\checks_z.py z14
  EXPECT: Z14 PASS gamma-bind-cover-correct
  EVIDENCE: exit=0 expect-matched sha256:17ee6439c2ac 1.4s

- [x] Z15: the incumbent arm still reproduces its known-good reference, confirming the evaluation path is intact
  CHECK: C:\ermal\Virtual_Environment_LocalSolver_3\Scripts\python.exe tools\checks_z.py z15
  EXPECT: Z15 PASS incumbent-matches-preregistration
  EVIDENCE: exit=0 expect-matched sha256:3832a197f8c8 1.0s

- [x] Z16: no row claims a proven infeasibility, since Hexaly is a local-search solver and cannot certify one
  CHECK: C:\ermal\Virtual_Environment_LocalSolver_3\Scripts\python.exe tools\checks_z.py z16
  EXPECT: Z16 PASS no-false-proof
  EVIDENCE: exit=0 expect-matched sha256:cb6b70390ca7 1.8s

- [x] Z17: every cell writes a complete six-arm results row set with no fold missing
  CHECK: C:\ermal\Virtual_Environment_LocalSolver_3\Scripts\python.exe tools\checks_z.py z17
  EXPECT: Z17 PASS per-fold-outputs-complete
  EVIDENCE: exit=0 expect-matched sha256:c12d1bb221d5 1.0s
...
- [x] Z18: scoring is scoped per z so the daily_metrics layout-collision defect cannot recur
  ...
  EVIDENCE: exit=0 expect-matched sha256:b80cdd06ea88 47.4s
...
- [x] Z19: POST-RUN CORRECTION - the four folds are NESTED expanding windows, so the pre-registered "179 training days" is a sum over 51 distinct days and the z=6 coverage anchor was inflated (99.4% stated, 98.0% actual)
  ...
  EXPECT: Z19 PASS folds-nested-anchors-honest
  EVIDENCE: exit=0 expect-matched sha256:4182de06f360 12.1s
