...
Four operators act across those stages.
\begin{itemize}
\item \textsc{GreedyPrice} assembles several bundles per master solve from the products carrying the highest dual prices $\sigma_p$.
...
\item \textsc{Complete} extends a priced bundle to a full layout by assigning every remaining product to the feasible station sharing the greatest order weight.
\item \textsc{Descend} exchanges two products held at different stations under a first-improvement rule.
\end{itemize}
Supplementary Section~S5 gives the incremental swap evaluation and the division of the cap $B$ across the stages.

Three design points are not visible in the stage list. First, Stage 2 resolves the stall because the patterns of the \emph{whole completed layout} enter the pool, so the complementary columns the integer master needs arrive together rather than one at a time. Second, feasibility is guaranteed rather than repaired, since \textsc{Descend} penalises a cap violation at $\rho = \sum_{u \in U} w_u$, which exceeds the maximal possible visit saving, and the feasibility gate discards any completion that still breaches a limit, so the incumbent can only improve. Third, Stage 3 lets the integer master reassemble a layout from bundles drawn from different completions, which no single completion could produce.
