...
$s'(p),\,k$ & Legacy station of product $p$; relocation cap of the bounded-reassignment extension (Section~\ref{sec:reassign}) \\
\midrule
\multicolumn{2}{@{}l}{\emph{Decomposition (Section~\ref{sec:cg})}}\\
...
\midrule
\multicolumn{2}{@{}l}{\emph{Clustering heuristic (Section~\ref{sec:heuristic})}}\\
$N$         & Catalogue size $|P|$, the index of the three graph filters \\
$\mathrm{cnt}_{xy}$ & Number of orders containing both products $x$ and $y$ \\
$\mathrm{load}(s)$  & Workload $\sum_{p\in s}L_p/V_s$ of station $s$ \\
$\phi,\,\gamma,\,r,\,\beta$ & Frequency floor; co-occurrence floor; kept-edge ratio; community size bound \\
\bottomrule
\end{tabularx}}
\label{tab:notation}
\end{table}
...
\end{itemize}
The picking work is identical and the buffer occupancy 60\% lower. The model does not observe diversion traffic directly, so holding a station's assigned lines within its engineered daily budget serves as the operational proxy. Both effects enter the model below, visits in the objective and workload as a hard constraint.
...
\end{enumerate}
Algorithm~\ref{alg:cgdrive} composes them across the four stages.
...
A generator can easily produce correlation no warehouse would exhibit, so we checked the instances against the Company~A data on three properties (Table~\ref{tab:realism}). The synthetic order size is drawn uniformly from the integers 5 to 15, which reproduces the industrial median of 10 items. In the industrial data only 2.47\% of the product pairs that could co-occur ever do, a sparsity reproduced by drawing each common itemset from a narrow fraction $U[0.1,0.5]$ of the catalogue. Taking the 1\% most frequent pairs and measuring how often one product is ordered when the other is gives 8.89\% synthetically against 14.24\% industrially, so the synthetic correlation is less pronounced than the industrial and the instances do not favour the correlation-driven methods.
...
\textbf{Both literature baselines trail, and the objective is one plausible explanation.} Their gap to the reference widens with size, from a few percent at 50 SKUs to more than a quarter at 2,000, and neither returns fewer visits than the reference on a single instance. Objective, initialisation, constraint handling and implementation are not isolated from one another here: the baselines keep their own prescribed starts (Section~\ref{sec:init}), reach feasibility through a penalty and not structurally, and are our own Python implementations against two commercial engines, so part of the deficit may be ours rather than theirs. What weakens with scale is the conversion rate between the surrogate and the visit count, since a swap changes that count only when it removes an order's last product from a station or carries its first to a new one, and most swaps do neither.

\textbf{The heuristic's trade of quality for time does not merely improve with scale: it reverses.} It trails at 50 SKUs, closes to within a point and a half at 1,000, and at 2,000 returns fewer visits than the reference on all three instances in under a minute against twenty. What matters is the composition of the move, not its size. The heuristic commits a whole community to a station in one step, and that community is assembled from products ordered together, so a single step can remove an order's last product from a station. The genetic algorithm's crossover copies a block drawn without reference to co-occurrence and then repaired for capacity, which buys no more of this objective than a single-product swap. The reversal at 2,000 SKUs rests on the same three instances as the crossing above, so we report it as observed on this benchmark rather than a general size beyond which the heuristic leads.
...
\item $|P| = 21{,}874$ products on the 24 evaluated stations, which enter the solvers and the evaluation.
...
\item $15{,}975$ products free to relocate.
\end{itemize}
...
\caption{Station-visit reduction against the share of the catalogue relocated, on the industrial instance. The dashed line marks the 145,042 visits removed by the full re-optimisation. The benefit accrues with relocation effort, and no small subset of moves captures a disproportionate share of it.}
\label{fig:ksweep}
\end{figure}

Bounded re-slotting returns benefit sub-proportionally to effort at every cap (Figure~\ref{fig:ksweep}): at $k = 5{,}000$, which is $31.3\%$ of the $15{,}975$ movable products, the layout captures $19.2\%$ of the available reduction, about 5.6 visits per relocated product against at least $145{,}042/15{,}975 \approx 9.1$ for the full re-optimisation, and no cap shows an inflection point at which a small subset of moves captures a disproportionate share. The objective explains the asymmetry: a visit disappears only when an order loses its last product at a station, so relocating part of a co-ordered group yields nothing until the remainder follows. We cannot separate that structural reading from an effect of the search budget: every bounded run exhausted its hour against the ten hours of the full re-optimisation, and none converged, so how much of the shortfall a longer run would recover is not measured here. Every point of the trade-off is nonetheless deployable, all stations staying within capacity and workload and close to their legacy loads, so a site can set its relocation budget against available labour.
