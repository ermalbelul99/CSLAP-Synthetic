...
%%  -- version 1 (transfer submission)
%%  Taylor & Francis "Interact" layout, APA reference style.
%%
%%  Branched from IJPR_CSLAP_v4.tex, which remains the ground truth for
%%  every experimental value. Changes in this version: the contribution is
%%  reframed around the problem reformulation and the industrial
%%  validation (the three solution methods become supporting evidence);
%%  the body is compressed to the journal's 12,000-word ceiling; the
%%  community-bound calibration table moves to the supplement, where the
%%  full eight-cell sweep already lives. No experimental value changed.
%%
...
%%  together with this file and IJSSOL_CSLAP_v1.bib. On Overleaf, start
%%  from the "Taylor & Francis Interact (APA reference style)" template
%%  and replace its main.tex and .bib with these two files.
%%
...
% ---------------------------------------------------------------------
\begin{abstract}
In automated pick-and-pass warehouses, throughput is governed by mechanical station stops along the conveyor rather than picker travel distance. However, correlated storage models traditionally inherit distance-minimisation objectives from manual picking. We reformulate the problem to directly minimise station visits per order, subject to hard per-station slot capacity and workload budgets. Because an order's visit count changes only when it gains or loses an entire station, the objective is piecewise constant, causing standard mixed-integer programming and conventional metaheuristics to stall at scale. To address this structure, we investigate three practical methods: a set-variable formulation, a set-partitioning column generation approach, and a fast clustering heuristic. Computational experiments across 29 benchmark instances demonstrate that these structural methods scale effectively, consistently outperforming standard metaheuristics used in the literature, as catalogue size grows. Applied to an operating facility with 21,874 products and 26 picking stations, the set-variable model eliminates 13.7\% of station visits while strictly preserving workload balance across all stations, with benefits remaining robust on future demand. Finally, a bounded reassignment model shows how facilities can implement these gains gradually, balancing operational disruption against throughput improvement.
\end{abstract}
...
Solving that model at the scale of a real warehouse is difficult for two reasons. The objective counts discrete stops, so its value changes only when a whole order gains or loses a zone and relocating a single product usually leaves it unchanged, giving a search steered by the visit count almost no gradient to follow. The linear relaxation of the binary programme is meanwhile no stronger than the observation that every order visits at least one station, whatever the correlation structure (Supplementary Section~S6), so branch-and-bound starts from a root bound carrying nothing instance-specific to prune with. A preliminary run of that formulation on a commercial solver bore this out, returning after an hour with the root bound still at the number of orders and the layout it had been given unimproved.

We make three contributions. First, we pose CSLAP for pick-and-pass systems with station visits as the objective under hard per-station capacity and workload limits, replacing the travel-distance objective the correlated storage literature inherits from manual picking, and give the model in two equivalent forms, a binary encoding and a set-variable reformulation (Section~\ref{sec:model}). Second, we validate it on a real warehouse of 21,874 products and 26 stations, where the reduction holds on weeks the layout was not fitted to and every station stays inside its own workload budget, and add a bounded reassignment extension capping how many products may move, so the gain deploys gradually on an active facility (Section~\ref{sec:industrial}). Third, we establish which method delivers that result by comparing three solution approaches, the two model forms and a set-partitioning column generation, against a genetic algorithm (GA) and simulated annealing with correlated interchange (SA-C) over 29 instances under one protocol with paired tests (Section~\ref{sec:experiments}).
...
% =====================================================================
All three methods minimise station visits subject to the slot capacity~\eqref{eq:cap} and the workload cap~\eqref{eq:wl}, and differ in what they give up to do it. The set-variable form is the one we deploy, on the evidence of Section~\ref{sec:industrial}; the others establish whether that choice is right. Its binary twin runs on the same engine and budget, isolating the decision representation. The clustering heuristic returns a feasible layout in minutes with no certificate of quality, whereas the set-partitioning column generation searches at the level of whole product bundles and returns a stream of feasible layouts with a lower bound it reports as a diagnostic. The GA and SA-C baselines of Section~\ref{sec:baselines} provide the external comparison.
...
The frequency floor $\phi=\max(2,\lfloor N/100\rfloor)$ admits a product only if it appears in at least $\phi$ orders, anchoring the noise floor at 1\% of catalogue volume; the co-occurrence floor $\gamma=\max(2,\lfloor N/50\rfloor)$ admits a pair only if its products are ordered together at least $\gamma$ times; and the kept-edge ratio $r=0.1$ keeps a pair only if its co-occurrence reaches a tenth of the more frequent endpoint's own demand, removing edges inflated by one ubiquitous endpoint. The community size bound $\beta=\max\!\left(5,\big\lfloor 0.4\,\zeta+\tfrac12 \big\rfloor\right)$, with $\zeta=|P|/|S|$ the slot capacity of a station, prevents clusters no station can hold.
...
The last stage repairs workload by exchanging products rather than relocating them, because there is nowhere to relocate to: the benchmark instances give $\sum_s \zeta_s = |P|$ exactly and on the industrial site each station's capacity is the number of products it already holds, so an exchange leaves both product counts unchanged while moving only the workload. It repeatedly takes the station of greatest relative overload $\mathrm{load}(s)/T_s$ and applies the exchange that breaks the least co-occurrence weight, $\mathrm{aff}(p,s)+\mathrm{aff}(q,t)-\mathrm{aff}(p,t)-\mathrm{aff}(q,s)$ with $\mathrm{aff}(x,s)=\sum_{y\in s}\mathrm{cnt}_{xy}$, so feasibility is recovered at the least cost to the clustering. The loop stops when no such exchange exists, a termination condition rather than a feasibility guarantee, though no instance here reached it with a station still over its cap.
...
Four operators act across those stages: \textsc{GreedyPrice} assembles several bundles per master solve from the products carrying the highest dual prices $\sigma_p$; \textsc{ExactPrice} solves the subproblem of Section~\ref{sec:cgpricing} to supply the dual bound, returning a flag recording whether the call reached proven optimality, since only a call that did and came back empty may stop the loop; \textsc{Complete} extends a priced bundle to a full layout by assigning every remaining product to the feasible station sharing the greatest order weight; and \textsc{Descend} exchanges two products held at different stations under a first-improvement rule. Three design points are not visible in the stage list. Stage 2 resolves the stall because the patterns of the \emph{whole completed layout} enter the pool, so the complementary columns the integer master needs arrive together rather than one at a time. Feasibility is guaranteed rather than repaired, since \textsc{Descend} penalises a cap violation at $\rho = \sum_{u \in U} w_u$, which exceeds the maximal possible visit saving, and the feasibility gate discards any completion that still breaches a limit, so the incumbent can only improve. And Stage 3 lets the integer master reassemble a layout from bundles drawn from different completions, which no single completion could produce. Supplementary Section~S5 gives the incremental swap evaluation and the division of the cap $B$ across the stages.
...
\textbf{Both literature baselines trail, and the objective accounts for it.} Their gap to the reference widens with size, from a few percent at 50 SKUs to more than a quarter at 2,000, and neither returns fewer visits than the reference on a single instance. We attribute this primarily to the objective rather than their tuning, though initialisation is a factor too, since the baselines keep their own prescribed starts (Section~\ref{sec:init}) and the two effects are not isolated here. What weakens with scale is the conversion rate between the surrogate and the visit count, since a swap changes that count only when it removes an order's last product from a station or carries its first to a new one, and most swaps do neither.
...
% =====================================================================
We validate the framework on a real warehouse, Company~A, which operates 26 stations. The dataset consists of anonymised commercial operational records, order lines and station assignments, supplied by the partner company and used with its permission. It contains no personal data and the study involves no human or animal subjects, so no ethical approval was required. Two of them, S25 and S26, hold 1 and 2 locations: too few for a reassignment to change the visit count and too few to leave the workload anywhere to move, so they are left as they stand and the catalogue of $|P| = 21{,}874$ products sits on the other 24, which enter the solvers and the evaluation. Within those 24 a product keeps the location the site has already given it when it appears in at most five orders \emph{and} sits at one of the three static-shelving stations, whose slotting is not re-planned; a further 171 products, which survive only in orders the extract does not retain, are pinned the same way. That leaves $15{,}975$ products free to relocate and fixes $5{,}899$. Frozen products keep their slots and their workload share, so the solver receives only the remaining tolerance while the evaluation charges every station with its complete load on the complete order stream; the reductions below are therefore not inflated by the reduced decision pool.

Here the workload row \eqref{eq:wl} keeps the normalised form of Section~\ref{sec:model}, each station's budget $T_s$ being $110\%$ of the load $\sum_{p} L_p/V_s$ its movable products carried under the legacy placement, with $V_s$ that station's own calibrated speed. Dividing by $V_s$ is what makes the quantity mean the same thing at every station, because the technologies differ and a raw line count does not: the busiest station handles $98{,}394$ lines at a speed of $9.92$ and so carries a load of $9{,}923$, while another handles $119{,}185$ lines at $30.43$ and carries $3{,}917$. The Max WL column of Table~\ref{tab:industrial} reports that load. Because the budgets it is measured against still differ in size from station to station, we report alongside it each station's utilisation, its realised load as a percentage of its own legacy load, and the standard deviation of those percentages over the 24 evaluated stations; that denominator gives the legacy layout zero dispersion by construction, so the metric reads as departure from the balance the site operates.
...
Heuristic & 1{,}044{,}932 & 1.65 & 13.89 & \cellcolor{blue!12}\textbf{647} & 10{,}911 & 27.90 & None \\
...
\textbf{The community bound does not transfer to this site.} Section~\ref{sec:heuristic} sets $\beta$ from a single slot capacity, presuming interchangeable stations; here they hold between 8 and 2{,}575 movable products, and the rule applied to their mean returns $\beta=266$, which no repair can make feasible. Nor is the failure a matter of overshooting, because across the nineteen settings evaluated feasibility is not an interval (Supplementary Section~S7). We therefore select $\beta$ on a seven-point grid anchored on the mean capacity, of which exactly one setting is feasible, $\beta=33$, giving the heuristic row of Table~\ref{tab:industrial}, whose time entry charges the whole grid because that is the protocol a site would run. Two exploratory settings are better and feasible, at $1{,}030{,}978$ and $1{,}036{,}764$ visits, but no grid specifiable in advance selects them, so we report them as a limitation. On interchangeable stations none of this arises.
...
This journal applies the Taylor \& Francis ``share upon reasonable request'' data sharing policy. Both synthetic instance families are openly available, the 29 benchmark and 80 calibration instances, each with the generator, station counts and seeds that produced them, together with the per-instance column-generation convergence record of Supplementary Section~S8, at \href{https://anonymous.4open.science/r/Anonymized_Export_CSLAP_v2-01BC/README.md}{this repository}. % ACTION: replace with a persistent DOI (Zenodo/Figshare) before submission -- the guideline asks for a persistent identifier, and single-anonymous review makes the anonymised host unnecessary. The Company~A dataset and the dated extract behind Table~\ref{tab:temporal} cannot be made public because of commercial confidentiality, and may be requested from the authors under reasonable operational conditions. Hexaly and IBM CPLEX depend on commercial licences we cannot redistribute; the model statements and settings needed to rebuild the methods on another solver are given in Sections~\ref{sec:model}, \ref{sec:cg} and~\ref{sec:experiments}.


% ---------------------------------------------------------------------
%  ACTION REQUIRED BEFORE SUBMISSION
%  The journal requires a short biographical note (max 200 words) for
%  every author. Ermal Belul's entry below is drafted from known facts;
%  the other three are STUBS and must be completed by the authors
%  themselves. Keep each to roughly 50 words: the word budget is tight
%  (see IJSSOL_Instructions.md Part H).
% ---------------------------------------------------------------------
...
\medskip
\noindent\textit{Marwane Bouznif} is a machine learning and optimization engineer at Savoye. He holds a doctorate from the Universit\'{e} de Grenoble (2012) on constant-time generic algorithms for combinatorial problems in rotagraphs and fasciagraphs. His current work applies operations research and machine learning to automated warehousing, including sorting, robot fleet sizing and storage assignment.

\medskip
\noindent\textit{Dritan Nace} is Professor at the Universit\'{e} de technologie de Compi\`{e}gne and a member of the Heudiasyc laboratory (UMR CNRS 7253), where he leads the SCOP research team. His work addresses operations research and network optimization, in particular robust and survivable network design, resilient routing, and fair resource allocation.

\medskip
\noindent\textit{Antoine Jouglet} is Professor at the Universit\'{e} de technologie de Compi\`{e}gne and a member of the Heudiasyc laboratory (UMR CNRS 7253), in the SCOP research team. His work addresses combinatorial optimization and scheduling, with a focus on dominance rules, exact and hybrid solution methods, and their application to industrial logistics problems.
