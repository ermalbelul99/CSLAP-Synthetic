...
We make three contributions. (1) We pose CSLAP for pick-and-pass systems with station visits as the objective under hard per-station capacity and workload limits, replacing the travel-distance objective the correlated storage literature inherits from manual picking, and give the model in a binary and a set-variable form (Section~\ref{sec:model}). (2) We build three solution strategies on the structure of that objective, a set-variable formulation as the principal method, a set-partitioning column generation that searches by whole product bundles, and a fast clustering heuristic. We benchmark them on 29 instances under one protocol against a genetic algorithm (GA) and simulated annealing with correlated interchange (SA-C) from the literature, with the binary encoding kept as a representation control (Sections~\ref{sec:methods} and~\ref{sec:experiments}). (3) We validate on an operating warehouse of 21,874 products and 26 stations, where the reduction holds on weeks the layout was not fitted to and every station stays inside its own workload budget, and add a bounded reassignment extension so the gain can be taken in stages (Section~\ref{sec:industrial}).
...
Five stages take the orders to a layout (Figure~\ref{fig:heuristic_flow}). At placement a community goes to the station with room for it whole that already holds the largest share of its products in the incumbent layout, ties settled towards the lower load ratio; a community fitting nowhere whole is split. The incumbent is the live layout on the industrial case and the longest-processing-time (LPT) start of Section~\ref{sec:init} on the synthetic instances. Supplementary Section~S4 states the procedure as pseudocode.

The last stage repairs workload by exchanging products rather than relocating them, because every station is already full: the benchmark instances give $\sum_s \zeta_s = |P|$ exactly, and on the industrial site each station's capacity is the number of products it already holds. It repeatedly takes the station furthest over its cap, offers one of its heaviest products in exchange for a lighter one at a station that can absorb it, and among the admissible swaps applies the one that separates the fewest co-ordered pairs, so feasibility is recovered at the least cost to the clustering. It stops when every station is inside its cap or no admissible swap remains; that is a stopping rule and not a feasibility guarantee, though every run reported here ended with no station over its cap. Supplementary Section~S4 states the selection rule exactly.
...
\end{aligned}
\]
Encoding coverage per distinct support rather than per order keeps the subproblem's size tied to the number of distinct supports, not the raw order count, which grows without bound as the horizon lengthens. What limits the method is the number of pricing iterations reached inside the cap, not the cost of building the model (Section~\ref{sec:cgbound}); Supplementary Section~S5 gives the build and solve costs.
...
\begin{itemize}
\item \textsc{GreedyPrice} builds one bundle by adding products in the order of the price the master currently puts on them, stopping where the bundle stops paying for itself.
\item \textsc{ExactPrice} solves the pricing subproblem of Section~\ref{sec:cgpricing} as an integer programme and reports whether it proved optimality, since only a proven-empty call may stop the loop.
\item \textsc{Complete} turns a bundle into a full layout by placing every remaining product at a station that has room for it and shares the most orders with it.
\item \textsc{Descend} improves a layout by swapping pairs of products between stations whenever the swap lowers the visit count, any station pushed over its workload cap being penalised heavily enough that the descent never buys visits with feasibility.
\end{itemize}
...
Supplementary Section~S5 gives the exact rule of each operator, the incremental swap evaluation and the division of the cap $B$ across the stages.
...
Both are adapted in one respect. The visit objective is piecewise constant, so most local swaps leave it unchanged and a search guided by it alone stalls on a plateau. Both baselines therefore evaluate a surrogate adding capacity and workload penalties to the visit count together with a dispersion term penalising the separation of correlated items; that third term breaks the plateau, keeping the search moving while the visit count stands still. The penalty and dispersion weights were fixed in advance, not tuned, and we did not search over them, so what the baselines report is the published operators under one untuned constraint-handling scheme. Supplementary Section~S5 gives the operators, weights and parameters of both.
...
A single instance cannot tell whether a ranking reflects method quality or one favourable draw of the generator, so we evaluate every method over 29 independently generated instances on distinct seeds. There are twelve at 50 SKUs, ten at 500, four at 1,000 and three at 2,000, the count falling with size because a single cell at 2,000 SKUs costs more solver time than the whole 50-SKU block. All optimising methods run under a common per-size cap of 120, 300, 600 and 1,200 seconds, so every ranking is conditional on that cap. Every run was produced on the same 16-core host under the same Python 3.10 environment, each method receiving the same four-thread allocation, with the column generation on CPLEX 22.1.1, the two MILP representations on Hexaly 13.0, the baselines our own Python implementations, and one run per instance. The genetic algorithm is stochastic, and so is the column-generation drive, which mutates its warm start and samples swap partners; with a single run per instance their algorithmic variance is not measured here, so any difference at a size carrying few instances rests on single draws. Supplementary Section~S5 states the setup, stage budgets, baseline parameters and seeds in full.
...
% =====================================================================
We validate the framework on a real warehouse, Company~A, which operates 26 stations. Two of them, S25 and S26, hold 1 and 2 locations: too few for a reassignment to change the visit count and too few to leave the workload anywhere to move, so they are left as they stand and the catalogue of $|P| = 21{,}874$ products sits on the other 24, which enter the solvers and the evaluation. Within those 24 a product keeps the location the site has already given it when it appears in at most five orders \emph{and} sits at one of the three static-shelving stations, whose slotting is not re-planned; a further 171 products, which survive only in orders the extract does not retain, are pinned the same way. That leaves $15{,}975$ products free to relocate and fixes $5{,}899$. Frozen products keep their slots and their workload share, so the solver receives only the remaining tolerance while the evaluation charges every station with its complete load on the complete order stream; the reductions below are therefore not inflated by the reduced decision pool. Supplementary Section~S7 records the dataset, the residual resources and the site rules that fix them.
...
The column generation removes 5.8\% of the visits the site incurs today while disturbing its balance least. It ran on Company~A in its station-indexed form with the set-variable formulation's inputs, so the two solve the same constrained problem. Four elements differ from the homogeneous drive. The warm start is the live layout rather than an LPT partition. Priced bundles are installed by an ejection-style recompletion, which empties the target station, installs the bundle there and reinserts the displaced products at the stations sharing the most order weight with them. The stage budgets follow a different schedule, and a bound pass absent from the homogeneous one prices every station once more at the incumbent duals before the integer master. Supplementary Section~S5 gives both schedules. The run priced exactly throughout and used its full cap. Its layout shares the set-variable solution's feasibility profile, both exceeding the raw legacy load on ten stations and both inside the site's 10\% tolerance everywhere (Figure~\ref{fig:workload}b, c). The drive produces that profile: warm-starting from the live layout keeps it near the incumbent balance, and enforcing the workload cap inside pricing means no violating bundle is generated.
...
Methodologically, two extensions remain. Dual stabilisation \citep{dumerle1999stabilized, wentges1997weighted} is the first, and Section~\ref{sec:cgbound} gives the measured reason for preferring it to a stronger formulation. The master never priced out inside the cap, so the bound is limited by how far the dual iteration got and not by the relaxation's strength, and reaching the value of the full linear master would turn the matheuristic reading into certified gaps. A fuller account would isolate the drive's operators by ablation and carry an adaptive large neighbourhood search and an open-source constraint-programming model \citep{ropke2006alns, perron2023cpsat} as further baselines, the latter measuring the cost to a practitioner without a commercial licence.

At the modelling level, two limitations remain. The first is product geometry: replacing the unit count of a station's capacity with a per-product footprint $f_p$ in \eqref{eq:cap} would admit three-dimensional packing limits without changing the structure of the programme, and the same extension covers duplicating a high-demand product across stations, which the single-station assumption excludes. The second is the visit-to-throughput link: we balance workload, which lowers expected queueing, but do not model the queue, and feeding the optimised layouts into a discrete-event simulation would convert the planning figures of Section~\ref{sec:managerial} into measured throughput and open the way to stochastic demand and real-time routing. What these layouts are worth in throughput rather than stops is the question we leave open.
