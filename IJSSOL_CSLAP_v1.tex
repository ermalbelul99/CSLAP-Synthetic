...
The method also reports a lower bound, a visit count no layout can beat, which shows how much room a returned layout might still leave. It comes from the linear master and the pricing solve rather than from the search, so it holds whatever produced the layouts. Such a bound is normally available only once pricing has run out of improving bundles, which no run here reaches inside its time cap. A Farley bound covers that case \citep{farley1990note}: it takes the master value as it stands and subtracts the most that the bundles not yet generated could be worth. Supplementary Section~S5 derives it for both forms of the master.

The bound is worth having only at 50 SKUs. From 500 upward it is too weak to say anything about the layouts obtained, and the cause is the time cap, not the model: pricing was still finding improving bundles when the budget ran out on every instance (Supplementary Section~S8), so the subtraction is made at prices that had not settled. A stronger formulation would not repair that, whereas dual stabilisation would (Section~\ref{sec:conclusion}). We therefore report the bound as a diagnostic rather than a certificate, and Section~\ref{sec:experiments} measures quality against a per-instance reference instead of a certified optimum.
