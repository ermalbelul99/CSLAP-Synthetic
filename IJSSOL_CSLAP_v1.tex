...
The method also reports a valid lower bound on the optimal visit count. It follows from linear-programming duality on the restricted master together with the pricing solve's own dual bound, so it holds whatever heuristic produced the layouts; Supplementary Section~S5 states the aggregated and the station-indexed form and derives both. The bound applies to the optimal layout, not the returned one, which is why Section~\ref{sec:experiments} compares against a per-instance reference and not a certified optimum. Within these caps it is informative only at 50 SKUs; from 500 upward it is vacuous, and the reason is iteration count, not relaxation strength: pricing did not converge on any instance, so what is reported is a Farley bound \citep{farley1990note} at a truncated dual vector (Supplementary Section~S8 releases the convergence record). That is why dual stabilisation, and not a stronger formulation, is the remedy Section~\ref{sec:conclusion} names. The aggregated form serves the homogeneous family, a heterogeneous site reverting to the station-indexed master.
