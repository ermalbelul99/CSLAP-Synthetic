...
\item \textbf{Station assignment.} A station is admissible for a community when it has slot room for the whole community and its workload $\mathrm{load}(s)$ plus the community's stays within $T_s$. Communities are placed in decreasing order of cumulative demand, each at the admissible station holding the largest share of its products in the incumbent layout, ties settled towards the lower load ratio $\mathrm{load}(s)/T_s$; a community no station admits whole is split, heaviest product first, each product placed by the same rule. The incumbent is the live layout on the industrial case and the longest-processing-time (LPT) start of Section~\ref{sec:init} on the synthetic instances.
...
\item \textbf{\textsc{Descend}.} It moves by the same product exchange as the workload repair of Section~\ref{sec:heuristic}, which tight capacity forces on both, but it is steered by the objective rather than by feasibility: it accepts the first exchange that lowers the visit count plus $\rho$ times the total workload excess, with $\rho$ the total order weight, so no exchange can buy visits with feasibility. Each candidate is scored from per-station support counts rather than by rescanning the orders, and the descent polishes the warm start, each completed layout and the final incumbent.
\end{enumerate}
\begin{algorithm}[H]
