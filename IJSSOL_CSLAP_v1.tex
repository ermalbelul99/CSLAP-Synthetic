...
\item \textbf{\textsc{Descend}.} A first-improvement descent exchanges pairs of products held at different stations, which leaves every station's product count unchanged, and accepts an exchange when it lowers the visit count plus $\rho$ times the total workload excess, with $\rho$ the total order weight so that no exchange can trade feasibility for visits. The change induced by an exchange is scored from per-station support counts rather than by rescanning the orders, and the descent polishes the warm start, each completed layout and the final incumbent.
\end{enumerate}
\begin{algorithm}[H]
