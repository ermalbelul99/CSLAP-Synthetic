...
**Text values (Q-015).** A `text` value is copied verbatim from the cited source, with LaTeX markup kept as written. Never paraphrase. When the value spans consecutive lines, join them with a single space and cite the first line. A text value that ends its sentence keeps the sentence's final punctuation (Q-024). The comparison of A and B normalises whitespace only; in LaTeX sources a `~` (non-breaking space) counts as whitespace.
...
| `comp.budget.definition` | text | how the companion defines the per-station workload budget: the defining clause, copied verbatim under the Text values rule, never a shortened phrase (F-089) |
