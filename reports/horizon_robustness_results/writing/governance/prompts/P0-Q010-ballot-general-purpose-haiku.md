...
**Your seat:** `general-purpose/haiku`, seat `red`, evidence slice `S2`. You are **read-only**: write nothing and return one ballot in reply.
...
**Red seat duty:** before your positions, confirm or amend the conservative ordering stated on the card (one line per addendum), then write the RED CASE against the leading option for each addendum.
