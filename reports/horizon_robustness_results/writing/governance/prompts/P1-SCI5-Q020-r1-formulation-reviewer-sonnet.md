...
**Your seat:** `formulation-reviewer/sonnet`, seat 3 of 5, role `plain`, evidence slice S4 (model and conditional guarantee). You are **read-only**: write nothing and return one ballot in reply.

**Seat record:** Plan SCI-5 default seat. Ballot weight under U7: 1.
...
## Your evidence slice S4 (model and conditional guarantee); read in depth

- Claims C-11, C-12, C-16, C-22 and C-23 in `W/evidence/claims.json`: what the robust rows guarantee, and under which assumption.
- `reports/horizon_robustness_results/EXPERIMENT_REVIEW_HANDOFF.md` §6 (the two-sided model and its uncertainty set).
