...
**Your seat:** `general-purpose/sonnet`, seat 5 of 5, role `plain`, evidence slice S5 (practitioner reading). You are **read-only**: write nothing and return one ballot in reply.

**Seat record:** Plan SCI-5 default is general-purpose/haiku as practitioner reader; U7 excludes haiku. general-purpose/opus built the claim register, so under §4.4 rule 1 the seat goes to general-purpose/sonnet, which built no artifact under decision. Ballot weight under U7: 1.
...
## Your evidence slice S5 (practitioner reading); read in depth

- Claims C-01, C-13 and C-30 in `W/evidence/claims.json`.
- `reports/horizon_robustness_results/EXPERIMENT_REVIEW_HANDOFF.md` §2 (the user's statement of the site's aim) and §11.
- Read as a site manager deciding whether to deploy a layout: what would you conclude from each wording?
