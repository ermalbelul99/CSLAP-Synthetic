...
**Your seat:** `positioning-reviewer/opus`, seat 4 of 5, role `blue`, evidence slice S3 (companion and positioning). You are **read-only**: write nothing and return one ballot in reply. As the blue seat, write the strongest defensible version of C-17's classification for your position.
...
## Your evidence slice S3 (companion and positioning); read in depth

- `IJSSOL_CSLAP_v1.tex` and `IJSSOL_CSLAP_v1_supplementary.tex`: how the companion frames its robustness scope and its limitations.
- Claims C-03, C-08, C-21 and C-24 in `W/evidence/claims.json`.
