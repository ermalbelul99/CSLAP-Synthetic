...
**Your seat:** `results-integrity-reviewer/opus`, seat 1 of 5, role `plain`, evidence slice S1 (tables and anchors). You are **read-only**: write nothing and return one ballot in reply.

**Seat record:** Plan SCI-5 default seat. Ballot weight under U7: 2.
...
## Your evidence slice S1 (tables and anchors); read in depth

- `W/evidence/anchors.json`: the held-out pass counts (`ho.pass.<ARM>.count`), departures below and above the modelled set (`ho.departures.*`), largest deviations (`ho.maxdev.*`), minimum training slack (`ho.min_slack.*`, `ho.incumbent.training_slack.*`) and the two-sided screen pass counts (`ts.berner.*`).
- `W/evidence/ANCHOR_SPEC.md` sections A and B, for what each key measures.
- `reports/horizon_robustness_results/tables/case_frame.csv`, the rows of campaign `ho3_20260913`.
