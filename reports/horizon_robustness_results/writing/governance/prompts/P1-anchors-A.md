...
# Brief: P1 anchors, builder A (double-blind reproduction)

**Identity:** `general-purpose`/opus, role `builder`. Return in reply. Write nothing to disk.

**Repository root:** `C:\ermal\CSLAP_Full_Project\CSLAP-Synthetic`

**Python:** `C:\ermal\Virtual_Environment_CPLEX_1\Scripts\python.exe` (pandas and numpy available)

## Blindness

A second builder is computing the same anchors independently.
- Do not open `reports/horizon_robustness_results/writing/evidence/independent/`.
- Do not open `reports/horizon_robustness_results/writing/tools/recompute_anchors.py` if it exists.
- Do not open any file under `reports/horizon_robustness_results/writing/governance/results/`.
...
2. Inspect the allowed inputs read-only. For each key, discover the columns and fields you need, and record every selector.
3. Write one self-contained Python script:
   - It uses only the standard library, pandas and numpy.
   - It reads only the allowed inputs.
   - It computes every key in ANCHOR_SPEC sections A–D.
   - It prints a single JSON object, `{key: entry}`, to stdout in the entry format of the spec, including a `sha256` for every source file.
   - It uses exact rational arithmetic (`fractions.Fraction`) wherever an exact field or a count exists.

   You may run it from a temporary directory, outside the repository or under your own session scratchpad, printing to stdout only.
4. Run the script and capture its output.
5. Check your ANCHOR_SPEC section E values against the displayed values. Report any mismatch in your reply; do not adjust your computation to match.
