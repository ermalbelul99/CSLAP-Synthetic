...
# Brief: anchors builder A, targeted revision 2 (findings F-078, F-079, F-080; revision of dispatch seq 37)
...
1. Read your own script, `W/tools/recompute_anchors.py`. It is your seq 37 revision, byte for byte. You may also read your own output, `W/evidence/anchors.json`.
2. Copy the script into your private directory, `C:\Users\ebelul\AppData\Local\Temp\2\p1a_rev2\`, and revise it there. Create the directory if it is missing; it is yours alone.
...
4. Run the revised script with `C:\ermal\Virtual_Environment_CPLEX_1\Scripts\python.exe -B`. Save the script as `p1a_rev2\anchors_A_rev2.py` and its stdout as `p1a_rev2\out_A_rev2.json`.
...
- anything under `C:\Users\ebelul\AppData\Local\Temp\2\p1b_rev\`, `p1b_rev2\` or `p1b_retry\`.
...
- Path and SHA-256 of `anchors_A_rev2.py` and of `out_A_rev2.json`.
- The key count (it was 208).
...
- A statement that every other entry is unchanged, checked by comparing `out_A_rev2.json` with `W/evidence/anchors.json` key by key.
