...
User decisions after the go are in `reports/horizon_robustness_results/writing/governance/user_decisions.md`. Under U5, IJSSOL v1 is the only baseline: never read or use any `IJPR_CSLAP_*` or `C&OR_CSLAP.tex` file.
...
# Brief: P1 document values, extractor B (double-blind reproduction)

**Identity:** `general-purpose`/opus, role `extractor`. Return your result in reply and write nothing to disk.
...
1. Read `reports/horizon_robustness_results/writing/evidence/DOCUMENT_VALUES_SPEC.md` in full.
2. For every key, find the value in the allowed sources. Record the exact 1-based line number and a verbatim substring of that line.
3. Compute the SHA-256 of each source file with a read-only command.
4. If a key has no clear source, set `"value_exact": "UNAVAILABLE"` and state which passages you searched.

## Return in reply

1. `OUTPUT`: one fenced json block holding the complete object `{key: entry}`.
2. `NOTES`, at most 250 words: ambiguities, whether `prov.stream_end` is an index or a length (with the reason), and any Q-cards.
