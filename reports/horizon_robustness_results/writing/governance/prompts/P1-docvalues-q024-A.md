...
# Brief: P1 document values, blind re-extraction of one key under the clarified text rule (Q-024)

**Identity:** `optimization-coder`/sonnet, role `extractor`, blind extraction A. This task reads documents only; ignore your role file's solver and coding defaults. Return everything in your reply and write nothing.
...
1. Read `reports/horizon_robustness_results/writing/evidence/DOCUMENT_VALUES_SPEC.md`: the entry format, the **Text values** rule (including its last sentence, on sentence-final punctuation), the search rule, and the row for `comp.budget.definition`.
2. Find the text that row asks for in the allowed sources.
3. Set `value_exact` to that text **verbatim**, with LaTeX kept exactly as written, applying every sentence of the Text values rule. If the text spans consecutive lines, join them with single spaces and cite the **first** line.
...
A fenced json block holding exactly one key, `comp.budget.definition`, with an entry in the spec's format. Then NOTES of at most 80 words: the lines you considered, where the value starts and ends, and why.
