...
# Brief: P1 document values, blind re-extraction of one key (Q-015)

**Identity:** `optimization-coder`/sonnet, role `extractor`, blind re-extraction A. Ignore your role file's solver defaults: this task reads one document only.

(Dispatch copies: prefix the standard dispatch preamble, the U5 to U8 pointer and the identity line for A or B. A is `optimization-coder`/sonnet, role `extractor`. B is `general-purpose`/opus, role `extractor`. Both return in reply and write nothing.)
...
- Do not open anything under `reports/horizon_robustness_results/writing/governance/results/`.
- Do not open `reports/horizon_robustness_results/writing/evidence/document_values.json`.

## Task

1. Read `reports/horizon_robustness_results/writing/evidence/DOCUMENT_VALUES_SPEC.md`: the entry format, the **Text values (Q-015)** rule and the row for `comp.supp.workload.definition`.
2. In `IJSSOL_CSLAP_v1_supplementary.tex`, find where the supplement defines or measures station workload or the workload budget.
3. Set `value_exact` to that defining text **verbatim**, with LaTeX kept exactly as written. Take the whole clause or clauses that define workload and the budget, and no paraphrase. If the text spans consecutive lines, join them with single spaces and cite the first line.
4. Set `source.quoted_text` to a verbatim substring of the cited line. Compute `source.sha256` read-only, for example with `python -B`.

## Return in reply

A fenced json block holding one key, `comp.supp.workload.definition`, with an entry in the spec's format. Then NOTES of at most 120 words: every line you considered and why you chose the one you did.

**Rules:** write nothing, run no git, and never read `IJPR_CSLAP_*` or `C&OR_CSLAP.tex`.
