"""Blind double-reproduction, builder B (retry, dispatch seq 36).
...
for arm in ARMS_A:
    for seed in SEEDS:
        ...
        set_key(
            ...
            entry(ev["cap_violation_count"], float(ev["cap_violation_count"]), "count",
        ...
        set_key(
            ...
            entry(ev["floor_violation_count"], float(ev["floor_violation_count"]), "count",
...
# --- ho.incumbent.training_slack (Q-005) ---
slack_vals = set()
for (arm, seed), (data, path) in ho_index.items():
    v = data["reference_evaluation"]["validation"]
    slack_vals.add((v["minimum_required_slack_exact"], v["scenario_shares"][0]["label"],
                     v["scenario_shares"][0]["start"], v["scenario_shares"][0]["stop"]))
set_key(
    "ho.incumbent.training_slack",
    unavailable(
        "reference_evaluation.validation.minimum_required_slack_exact was inspected across all 12 "
        "ho3_20260913 case records. It is NOT a single stable incumbent value: it takes value 0 "
        "when reference_evaluation.validation.scenario_shares[0] has label='history', start=0, "
        "stop=243151 (matching the origin), but takes value 1117947199/71202343420 when "
        "scenario_shares[0] has label='block', start=2537, stop=24411 (a narrow sub-block) on other "
        "rows. Since the two candidate values disagree and neither is flagged in the artifact as the "
        "canonical incumbent training slack, this key is not confidently sourced."
    ),
)
